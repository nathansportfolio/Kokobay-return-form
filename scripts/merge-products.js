...
/** Strip BOM / trim header keys so list-active CSV (e.g. `active-products-2.csv`) parses reliably. */
    ...
    .on("end", () => {
      ...
      Object.keys(grouped).forEach(productHandle => {
        ...
        group.forEach(row => {
          ...
          const sku = (
            safeString(row.sku || row["Variant SKU"] || "") ||
            `${safeString(productHandle).replace(/[^a-z0-9]/gi, "").toUpperCase()}-${String(color).toUpperCase()}-${size.replace(/[^a-z0-9]/gi, "")}`
          ).replace(/\s+/g, "-");

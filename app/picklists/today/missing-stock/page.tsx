...
import {
  listOrderPickPausesForDay,
} from "@/lib/orderPickPause";
...
export const metadata: Metadata = {
  title: "Missing stock — standard picks",
  description:
    "Orders paused from active pick lists because stock was missing at a bin",
};

export default async function StandardMissingStockPage() {
  const dayKey = getPickListOrderDayKey();
  let rows: Awaited<ReturnType<typeof listOrderPickPausesForDay>>;
  try {
    rows = await listOrderPickPausesForDay(dayKey, PICKLIST_LIST_KIND_STANDARD);
  } catch {
    return (
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 p-4 pb-12 sm:p-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">
          Missing stock (standard)
        </h1>
        <p className="text-sm text-zinc-600 dark:text-zinc-400">
          Orders on this list are{" "}
          <span className="font-medium text-foreground">not</span> on active pick
          lists until you clear the hold. They are still{" "}
          <span className="font-medium text-foreground">not</span> marked as picked.
        </p>
        <p className="text-sm text-zinc-500">
          ...
          (yesterday, London — same as standard pick lists).
        </p>
        <Link
      ...
      {rows.length === 0 ? (
        <div className="rounded-xl border border-dashed border-zinc-300 bg-zinc-50/80 p-8 text-center dark:border-zinc-600 dark:bg-zinc-950/40">
          <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
            No orders on hold for missing stock
          </p>
        </div>
      ) : (
